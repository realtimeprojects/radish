...
class ANSISequenceNotFoundException( Exception ):
  ...
  def __str__( self ):
    return "ANSI Sequence `{}` could not be found in `{}`".format( self.name, self.modetype )
...
class ColorfulMeta( type ):
  class ColorfulOut( type ):
    def __getattr__( cls, attr ):
      def decoratedText( text ): print( ColorfulParser.ParseAttr( attr )%( text ))
      return decoratedText

  out = ColorfulOut( "out", (object, ), {} )

  def __getattr__( cls, attr ):
    def decoratedText( text ): return ColorfulParser.ParseAttr( attr )%( text )
    return decoratedText
